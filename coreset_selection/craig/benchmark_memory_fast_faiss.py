#!/usr/bin/env python
"""
Craig Memory Constraint Experiment - FAISS Version (V2)

Compare FAISS-based in-memory optimized with NearPy-based version.
This script runs the FAISS backend to show that index-based ANN 
has lower memory overhead than hash-based ANN (NearPy).
...
Usage:
  python run_memory_experiment_v2.py                    # Run FAISS optimized
  python run_memory_experiment_v2.py --memory-tag 200m  # With memory tag
"""
import os
import sys
...
# Only run FAISS-based in-memory optimized
# (We already have NearPy data from the original experiments)
SETUPS = {
    'in_memory_optimized_faiss': {'backend': 'faiss', 'use_ann': True, 'force_backend': True},
}
...
def main():
    parser = argparse.ArgumentParser(description='Craig Memory Experiment - FAISS V2')
    ...
    print("="*70)
    print("Craig Memory Constraint Experiment - FAISS V2")
    print("="*70)
