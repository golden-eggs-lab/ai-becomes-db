...
# These are only used in some old utility functions, not needed for CRAIG
...
def get_facility_location_submodular_order(S, B, c, smtk=0, no=0, stoch_greedy=0, weights=None, 
                                           use_ann=False, gradients=None, ann_k=20, ann_backend='nearpy', 
    ...
    - ann_backend: str, 'nearpy', 'milvus', or 'spark' (default 'nearpy')
    ...
    if smtk > 0:
        ...
        if (use_ann or force_backend) and gradients is not None:
            ...
            # Use batch strategy for Spark backend (reduces Spark calls)
            if ann_backend == 'spark':
    ...
    # Weight assignment (Algorithm line 8) - optimization target
    ...
    collected = gc.collect()
    return order, sz, greedy_time, F_val, weight_time



def faciliy_location_order(c, X, y, metric, num_per_class, smtk, no, stoch_greedy, weights=None, 
                          use_ann=False, ann_k=20, ann_backend='nearpy', force_backend=False, batch_size=20, use_reuse=True):
    ...
    order, cluster_sz, greedy_time, F_val, weight_time = get_facility_location_submodular_order(
    ...
    return class_indices[order], cluster_sz, greedy_time, S_time, weight_time
...
def save_all_orders_and_weights(folder, X, metric='l2', stoch_greedy=False, y=None, equal_num=False, outdir='.'):
    ...
    for c in classes:
        ...
        start = time.time()
        if metric in ['', 'l2', 'l1']:
            dists = sklearn_pairwise_distances(X[class_indices[c]], metric=metric, n_jobs=1)
        else:
            p = float(metric)
            dim = class_nums[c]
...
def get_orders_and_weights(B, X, metric, smtk, no=0, stoch_greedy=0, y=None, weights=None, equal_num=False, 
                          outdir='.', use_ann=False, ann_k=20, ann_backend='nearpy', force_backend=False, batch_size=20, use_reuse=True):
    '''
    Ags
    - X: np.array, shape [N, d]
    - B: int, number of points to select
    ...
    # pool.terminate()
    order_mg_all, cluster_sizes_all, greedy_times, similarity_times, weight_times = zip(*map(
    ...
    total_ordering = sum(greedy_times)
    total_similarity = sum(similarity_times)
    total_weight = sum(weight_times)
    total_all = total_ordering + total_similarity + total_weight
    print(f"\n=== TIME BREAKDOWN (sum across {len(classes)} classes) ===")
    print(f"  Ordering (greedy):  {total_ordering:.2f}s ({100*total_ordering/total_all:.1f}%)")
    print(f"  Similarity (matrix): {total_similarity:.2f}s ({100*total_similarity/total_all:.1f}%)")
    print(f"  Weight assignment:  {total_weight:.2f}s ({100*total_weight/total_all:.1f}%)")
    print(f"  TOTAL measured:     {total_all:.2f}s")
    ...
    weight_time = np.max(weight_times)  # Algorithm line 8 - weight assignment
    ...
    vals = order_mg, weights_mg, order_sz, weights_sz, ordering_time, similarity_time, weight_time
    return vals
