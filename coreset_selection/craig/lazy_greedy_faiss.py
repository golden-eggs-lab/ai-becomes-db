...
# FAISS library (index-based ANN)
try:
    import faiss
    FAISS_AVAILABLE = True
except ImportError:
    FAISS_AVAILABLE = False
    print("Warning: FAISS not available. FAISS backend disabled.")

# Keep other backends for compatibility
try:
...
class FacilityLocationANN:
    ...
    V2: Uses FAISS (index-based) instead of NearPy (hash-based).
    
    Key difference from v1 (NearPy):
    - FAISS uses inverted file index (IVF) which has lower memory overhead
    - NearPy uses LSH hash tables which require more memory for random projections
    - Under memory pressure, FAISS should degrade more gracefully
    """
    
    def __init__(self, D, V, gradients, alpha=1., ann_k=10, use_ann=True, ann_backend='faiss', force_backend=False, use_reuse=True):
        '''
        Args
        - D: np.array, shape [N, N], similarity matrix
        - V: list of int, indices of columns of D
    ...
    def _check_backend_available(self):
        ...
        if self.ann_backend == 'faiss':
            if not FAISS_AVAILABLE:
                print(f"Warning: FAISS not available, falling back to exact search")
                return False
            return True
        elif self.ann_backend == 'milvus':
    ...
    def _init_faiss_engine(self):
        """Initialize FAISS index-based ANN engine.
        
        FAISS advantages over NearPy (LSH):
        - Lower memory overhead: IVF uses centroids, not random projections
        - Better accuracy-speed tradeoff with nprobe parameter
        - More memory-efficient under pressure
        """
        print(f"  Building FAISS index...")
        start = time.time()
        
        # Normalize gradients for cosine similarity (FAISS works with L2/IP)
        # We'll use L2 distance since original code uses euclidean
        gradients_float32 = self.gradients.astype(np.float32)
        
        if self.use_ann:
            # ANN mode: Use IVF (Inverted File) index for approximate search
            # IVF partitions the space into nlist clusters
            nlist = min(100, self.N // 10)  # Number of clusters
            nlist = max(4, nlist)  # Minimum 4 clusters
            ...
            print(f"    nprobe={self.faiss_index.nprobe}")
        else:
            # Exact mode: Use flat index (brute force)
        ...
        print(f"FAISS engine initialized with {self.N} vectors")
    ...
    def _init_milvus_engine(self):
        ...
        import os
        db_path = os.path.abspath("./milvus_craig_v2.db")
    ...
    def find_best_candidate_ann(self, sset, candidates):
        ...
        if self.ann_backend == 'faiss':
            ann_candidates = self._faiss_search(R, candidates)
        elif self.ann_backend == 'milvus':
    ...
    def _faiss_search(self, query_vector, candidates):
        """
        FAISS-based ANN search.
        
        Uses IVF index for ANN mode, flat index for exact mode.
        Lower memory overhead compared to NearPy's hash tables.
        """
        # FAISS requires float32 and 2D array
        query = query_vector.astype(np.float32).reshape(1, -1)
        
        # Search for k nearest neighbors
        k = min(self.ann_k * 2, self.N)  # Search for more to filter by candidates
        distances, indices = self.faiss_index.search(query, k)
        
        # Extract candidate indices from FAISS results
        ann_candidates = []
        candidates_set = set(candidates)
        target_count = self.ann_k if self.use_ann else len(candidates)
        
        for idx in indices[0]:
            if idx >= 0 and idx in candidates_set:  # FAISS returns -1 for empty slots
...
def lazy_greedy_heap_ann_batch(F, V, B, batch_size=20, use_ann=True):
    ...
    BATCH STRATEGY:
    - Every batch_size selections, call ANN backend to get ~ann_k candidates
    ...
    - Both call backend same number of times (B / batch_size)
    - Only difference: backend uses ANN (approximate) vs full distance (Exact)
    
    Args:
    - F: FacilityLocationANN object
    - V: list of candidate indices
    - B: budget (number of elements to select)
    - batch_size: how many selections per backend call (default 20)
    - use_ann: whether to use ANN for backend search
    ...
    print(f"Expected backend calls: {(B + batch_size - 1) // batch_size}")
    ...
    while len(sset) < B and remaining:
        ...
        # ========== Step 1: Backend query to get candidates ==========
        backend_start = time.time()
        R = F.get_residual()  # O(d) - just returns cached residual
        
        # Backend narrows down: remaining (e.g., 6000) → candidates (e.g., 100)
        if F.ann_backend == 'faiss' and F.use_backend:
            candidates = F._faiss_search(R, remaining)
        elif F.ann_backend == 'milvus' and F.use_backend:
            candidates = F._milvus_search(R, remaining)
        elif F.ann_backend == 'spark' and F.use_backend:
        ...
        while batch_selected < batch_size and len(sset) < B and candidates_set:
            ...
            for idx in candidates_set:
                gain = F.inc(sset, idx)  # True marginal gain
                if gain > best_gain:
                    best_gain = gain
                    best_idx = idx
        ...
        print(f"Batch {batch_num}: selected {batch_selected} elements, "
              ...
              f"backend={backend_elapsed:.2f}s, python={python_elapsed:.3f}s")
    
    # Final statistics
    print(f"\n=== BATCH Statistics ===")
    print(f"Total backend queries: {F.ann_queries}")
    print(f"Total backend time: {backend_time:.2f}s")
    print(f"Total Python time: {python_time:.2f}s")
    print(f"Avg backend time per query: {backend_time/F.ann_queries*1000:.1f}ms" if F.ann_queries > 0 else "N/A")
