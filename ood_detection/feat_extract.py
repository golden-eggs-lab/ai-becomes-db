...
for split, in_loader in [('train', trainloaderIn), ('val', testloaderIn),]:
    ...
    if FORCE_RUN or not os.path.exists(cache_name):
        ...
        save_data = np.array([feat_log.T, score_log.T, label_log], dtype=object)
        np.save(cache_name, save_data)
    else:
...
for ood_dataset in args.out_datasets:
    ...
    if FORCE_RUN or not os.path.exists(cache_name):
        ...
        save_data = np.array([ood_feat_log.T, ood_score_log.T], dtype=object)
        np.save(cache_name, save_data)
    else:
